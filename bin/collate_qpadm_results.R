...
suppressWarnings({
  for (i in seq_along(r)) {
    ...
    wave_ranks_perm <- paste(r[[i]]$permutation_wave_rank_estimates,
        collapse = "|")
    wave_ranks_perm_mt <- paste(r[[i]]$permutation_wave_rank_estimates_mt,
        collapse = "|")
    left <- paste(r[[i]]$left, collapse = "|")
    right <- paste(r[[i]]$right, collapse = "|")
    target <- r[[i]]$target
    r[[i]]$popdrop %<>% mutate(i = i, adm_rank = adm_rank,
        wave_rank = wave_rank, wave_ranks_perm = wave_ranks_perm,
        wave_ranks_perm_mt = wave_ranks_perm_mt,
        pattern = paste("p", pat, sep = "="), left = left, right = right,
        target = target)
...
setcolorder(t, c(
    "i", "adm_rank", "wave_rank", "wave_ranks_perm", "wave_ranks_perm_mt",
    "pat", "pattern", "wt", "dof", "chisq", "p", "f4rank", "feasible", "best",
    "dofdiff", "chisqdiff", "p_nested"
))
